...
export function SkeletonUtilsClone(source) {
	return clone(source)
}

function getSkeletonKey(skeleton) {
	...
	function traverse(bone) {
		return {
			...
			children: bone.children
				.filter(child => bones.includes(child))
				.map(traverse)
		};
	}
	const boneSet = new Set(bones);
	const rootBones = bones.filter(bone => !bone.parent || !boneSet.has(bone.parent));
	return JSON.stringify({
		hierarchy: rootBones.map(traverse),
		inverses: inverses?.map(m => m.elements).flat().slice(0, 10) // hash or partial
	});
}
...
function clone( source ) {
	...
	clone.traverse( function ( node ) {
		...
		//this part is added for studio
		const skeletonKey = getSkeletonKey(sourceMesh.skeleton);
		const doesExistSkeleton = skeletonLookup.get(skeletonKey);
		if (doesExistSkeleton) {
			clonedMesh.bindMatrix.copy( sourceMesh.bindMatrix );
			clonedMesh.bind( doesExistSkeleton, clonedMesh.bindMatrix );
		} else {
			...
			// skeletonLookup.set(sourceMesh.skeleton.uuid, clonedMesh.skeleton)
			skeletonLookup.set(skeletonKey, clonedMesh.skeleton)
